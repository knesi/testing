...
NSString * SZJaegercorps$ModuleRequireFormat = @"(function(exports){"
		"var __OXP=exports;var module={'exports':exports};%@;\n"
		"if(module.exports !== __OXP){return module.exports;}"
		"return exports;})({})";


@implementation SZJaegercorpsObject
...
-(void)injectPatches
{
	// called to inject any SZJaegercorps patches in JS before a context is loaded... nice for 
...
{
	// create SZJaegercorps global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_szjaegercorps = [[SZJaegercorpsObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_szjaegercorps];
	
	NSString *_szjaegercorpsNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _szjaegercorpsNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_szjaegercorps valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:SZJaegercorps$ModuleRequireFormat,code];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	if (data!=nil)
	{
		...
		if (![module respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			@throw [NSException exceptionWithName:@"org.szjaegercorps.kroll" reason:[NSString stringWithFormat:@"Module \"%@\" failed to leave a valid exports object",path] userInfo:nil];
		}
	...
	@throw [NSException exceptionWithName:@"org.szjaegercorps.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
